    ...
    <article class="forms">
      <section class="forms__item" v-for="form in filteredForms" :key="form">
        ...
        >
          ...
          {{ form.date }}
          <h2>Pełniona rola LSPD:</h2>
          {{ form.act }}
          <h2>Co możesz wnieść do LSPD:</h2>
          {{ form.bring }}
          <h2>Dlaczego ty?:</h2>
          {{ form.whyU }}
          <h2>Wiek:</h2>
          ...
          {{ form.hoursPerDay }}
          <h2>Doświadczenie w frakcji LSPD:</h2>
          {{ form.experience }}
          <h2>Krótka scenka</h2>
          {{ form.action }}
          <h2>Discord:</h2>
...
export default {
  ...
  created() {
    Axios.post(`${process.env.VUE_APP_API_URL}/admin/lspd`, {
  ...
  methods: {
    ...
    alert(type) {
      ...
      if (type === 'accept') {
        ...
        Axios.post(`${process.env.VUE_APP_API_URL}/admin/lspd/check`, {
          token: this.$store.state.token,
          id: this.id,
          status: 'accepted',
        });
